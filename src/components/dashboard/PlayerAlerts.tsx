  ...
  if (injuredPlayers.length === 0 && focusPlayers.length === 0) {
    return (
      <div className="stat-card">
        <h3 className="metric-label mb-3">Spelarnotiser</h3>
      ...
      <div className="space-y-0 divide-y divide-border">
        {injuredPlayers.map(player => (
          <div 
            ...
            className={cn("flex items-center justify-between py-2", onPlayerClick && "cursor-pointer hover:bg-muted/50")}
          >
            <div className="min-w-0">
              <p className="text-sm font-medium text-foreground">{player.name}</p>
              {player.notes && <p className="text-xs text-muted-foreground truncate">{player.notes}</p>}
            </div>
            <span className="status-badge status-injured shrink-0 ml-2">Skadad</span>
          </div>
        ))}
        ...
        {focusPlayers.map(player => (
          ...
          >
            ...
            <span className="status-badge status-focus shrink-0 ml-2">Fokus</span>
          </div>
        ))}
      </div>
